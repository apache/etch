...
public class Signal extends Opt
{
	public Signal( Name name, Token... args ) throws ParseException
	{
		...
		if (args.length != 1)
			throw new ParseException( String.format( "Signal args length != 1" ) );
		
		Token arg = args[0];
		if (arg.kind != EtchGrammarConstants.ID)
			throw new ParseException( String.format( "Signal arg should be identifier: "+arg.image ) );
		
		this.name = new Name(arg, arg.image);
	}
	...
	{
		System.out.printf( "%s@Signal( %s )\n", indent, name );
	}

	@Override
	public String toString()
	{
		return String.format( "@Signal( %s )", name );
	}
