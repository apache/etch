...
{
    public class TlsConnection : Connection<SessionData>, TransportData
    {
      
        /// <summary>
        /// Term on the uri which specifies the reconnect delay.
        /// </summary>
        public readonly static String RECONNECT_DELAY = "TlsConnection.reconnect_delay";

        /// <summary>
        /// Term on the uri which specifies the default auto flush flag.
        /// </summary>
        private static readonly String AUTO_FLUSH = "TlsConnection.autoFlush";

        /// <summary>
        /// Term on the uri which specifies the default keep alive flag.
        /// </summary>
        private static readonly String KEEP_ALIVE = "TlsConnection.keepAlive";

        /// <summary>
        /// Term on the uri which specifies the default linger time.
        /// </summary>
        private static readonly String LINGER_TIME = "TlsConnection.lingerTime";

        /// <summary>
        /// Term on the uri which specifies the default no delay flag.
        /// </summary>
        private static readonly String NO_DELAY = "TlsConnection.noDelay";

        /// <summary>
        /// Term on the uri which specifies the default traffic class.
        /// </summary>
        private static readonly String TRAFFIC_CLASS = "TlsConnection.trafficClass";

        /// <summary>
        /// Term on the uri which specifies the default buffer size parameter.
        /// </summary>
        private static readonly String BUFFER_SIZE = "TlsConnection.bufferSize";

        /// <summary>
        /// Term on uri which specifies whether server certificate should be authenticated. 
        /// </summary>
        private static readonly string CERT_AUTH_REQUIRED = "TlsConnection.authReqd";

        /// <summary>
        /// Term on uri which specifies the certificate name of the server.
        /// </summary>
        private static readonly string CERT_NAME = "TlsConnection.certName";


        private SslStream sslStream = null;

        private string certName = "default";

        /// <summary>
        /// This flag will only be accessed by the client connection, it is always true for server or listener connection.
        /// </summary>
        private bool isAuthReqd = true;
          ...
          
        {
            ...
            string cert = u.GetTerm(CERT_NAME);
            if (cert != null)
            ...
            if (socket == null)
            {
               ...
               isAuthReqd = (bool) u.GetBooleanTerm(CERT_AUTH_REQUIRED,true);
            ...
            else
            {
                ...
                try
                {
                    sslStream.AuthenticateAsServer(GetServerCert(this.certName));
                }
        ...
        public override string ToString()
        {
            ...
            if (s != null)
                return String.Format("TLS (up, {0})", s.LocalEndPoint);

            return String.Format("TLS (down, {0}, {1})", host, port);
        }
        ...
        public void SetCertificateName(string cert)
        {
            this.certName = cert;
        }
        ...
        protected override bool OpenSocket(bool reconnect)
        {
            ...
            while (IsStarted())
            {
               ...
               /*         Console.WriteLine("Exception: {0}", e.Message);
                        ...
                        AuthenticationException e1 = new AuthenticationException("Remote Certificate Mismatch Error. Problem in setting up SSL Connection. Either disable" +
				 " server authentication by using flag TlsConnection.authReqd=false in uri or provide" +
				" a a valid certificate name in TlsConnection.certName flag on uri",e.InnerException);
                       
                        FireException("open",e1);
        ...
        protected override void ReadSocket()
        {
         ...
         //           n = socket.Receive(buf.GetBuf(), count, buf.Length() - count, SocketFlags.None);
                    ...
                    n = sslStream.Read(buf.GetBuf(), count, buf.Length() - count);
