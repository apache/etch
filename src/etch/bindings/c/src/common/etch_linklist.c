...
{
    etch_linklist* list = etch_malloc(sizeof(etch_linklist), ETCHTYPEA_COLLECTION);
...
{    
    if (NULL == list) return;
    linkist_clear(list, is_free_content);
    etch_free(list);
}
...
int linklist_add(etch_linklist* list, void* content) 
{
    ...
    node = etch_malloc(sizeof(linklist_node), ETCHTYPEA_RAW);
...
int linklist_insert(etch_linklist* list, const unsigned int i, void* content)
{
    ...
    newnode = etch_malloc(sizeof(linklist_node), ETCHTYPEA_NODE);
