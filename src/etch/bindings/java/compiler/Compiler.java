...
public class Compiler extends Backend
{
	...
	@Override
	public void generate( Module module, Set<String> whatContainer, File dir,
		...
		throws Exception
	{
		...
		ReservedWordChecker checker = new ReservedWordChecker( words, false );
		module.treewalk( checker );
		if (!checker.ok())
		{
			System.err
				.printf( "Etch: encountered errors during java generation.\n" );
			System.exit( 2 );
		}
		...
		{
			gIntf = intf;
			generate( intf, whatContainer, dir );
		}
		gIntf = null;
	}
		...
		throws Exception
	{
		...
		generate( intf, dir, msgDir, MessageDirection.BOTH, false, hasBaseClass );
...
//		System.out.println( "force = "+force );
		...
		{
			doFile( dir, getHelperName( intf ) + fnSuffix, true, force, new Gen()
			{
				public void run( PrintWriter pw ) throws Exception
				{
//					System.out.println( "generating helper" );
	...
	{
		VelocityContext context = new VelocityContext();
		// put values in context context.put();
		context.put( "now", new Date() );
		context.put( "version", VERSION );
		context.put( "helper", this );
		context.put( "intf", intf );
	...
	public String getValidator( Named<?> named )
	{
		if (named instanceof Parameter)
		{
			...
			return "Validator_custom.getCustom( "
				+ type.getNamed( gIntf ).efqname( this ) + ".class, "
				+ type.dim() + " )";
		}
		...
		if (named instanceof Thrown)
		{
			...
			return "Validator_custom.getCustom( " + e.efqname( this )
				+ ".class, 0 )";
		}
