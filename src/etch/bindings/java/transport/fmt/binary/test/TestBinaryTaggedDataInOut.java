...
public class TestBinaryTaggedDataInOut
{
	...
	@Test public void test_check_value() throws Exception
	{
		...
		// struct
		Assert.assertEquals( TypeCode.CUSTOM, btdo.checkValue( new StructValue( new Type( "foo" ) ) ) );
...
//		System.out.println( "msg = "+msg );
		...
		res.put( "Messagizer.valueFactory", vf );
...
//		System.out.println( "msg = "+msg );
	...
	{
		assertValueToBytes( null, new byte[] { -128 } );
		
		assertValueToBytes( false, new byte[] { -126 } );
		assertValueToBytes( true, new byte[] { -125 } );
		
		// tiny int:
		assertValueToBytes( 0, new byte[] { 0 } );
		assertValueToBytes( 1, new byte[] { 1 } );
		assertValueToBytes( -1, new byte[] { -1 } );
		
		// byte:
		assertValueToBytes( -100, new byte[] { -124, -100 } );
		
		// short:
		assertValueToBytes( 10000, new byte[] { -123, 39, 16 } );
		assertValueToBytes( -10000, new byte[] { -123, -40, -16 } );
		
		// int:
		assertValueToBytes( 1000000000, new byte[] { -122, 59, -102, -54, 0 } );
		assertValueToBytes( -1000000000, new byte[] { -122, -60, 101, 54, 0 } );
		
		// long:
		assertValueToBytes( 1000000000000000000L, new byte[] { -121, 13, -32, -74, -77, -89, 100, 0, 0 } );
		assertValueToBytes( -1000000000000000000L, new byte[] { -121, -14, 31, 73, 76, 88, -100, 0, 0 } );
		
		// float:
		assertValueToBytes( 12345f, new byte[] { -120, 70, 64, -28, 0 } );
		
		// double:
		assertValueToBytes( 23456d, new byte[] { -119, 64, -42, -24, 0, 0, 0, 0, 0 } );
		
		// boolean[]:
		assertValueToBytes( new boolean[] { true, false }, new byte[] { -111, -125, 1, 2, -125, -126, -127 } );
		
		// byte[]:
		assertValueToBytes( new byte[] { 1, 2, 3 }, new byte[] { -117, 3, 1, 2, 3 } );

		// byte[][]:
		assertValueToBytes( new byte[][] { new byte[] { 1, 2, 3 }, new byte[] { 2, 3, 4 } }, new byte[] { -111, -124, 2, 2, -117, 3, 1, 2, 3, -117, 3, 2, 3, 4, -127 } );

		// short[]:
		assertValueToBytes( new short[] { 1, 2, 3 }, new byte[] { -111, -123, 1, 3, 1, 2, 3, -127 } );

		// int[]:
		assertValueToBytes( new int[] { 1, 2, 3 }, new byte[] { -111, -122, 1, 3, 1, 2, 3, -127 } );

		// long[]:
		assertValueToBytes( new long[] { 1, 2, 3 }, new byte[] { -111, -121, 1, 3, 1, 2, 3, -127 } );

		// float[]:
		assertValueToBytes( new float[] { 1, 2, 3 }, new byte[] { -111, -120, 1, 3, -120, 63, -128, 0, 0, -120, 64, 0, 0, 0, -120, 64, 64, 0, 0, -127 } );

		// double[]:
		assertValueToBytes( new double[] { 1, 2, 3 }, new byte[] { -111, -119, 1, 3, -119, 63, -16, 0, 0, 0, 0, 0, 0, -119, 64, 0, 0, 0, 0, 0, 0, 0, -119, 64, 8, 0, 0, 0, 0, 0, 0, -127 } );

		// empty string:
		assertValueToBytes( "", new byte[] { -110 } );
		
		// string:
		assertValueToBytes( "abc", new byte[] { -109, 3, 97, 98, 99 } );
		
		// string[]:
		assertValueToBytes( new String[] { "a", "", "c" }, new byte[] { -111, -109, 1, 3, -109, 1, 97, -110, -109, 1, 99, -127 } );

		// Date:
		assertValueToBytes( new Date( 123456789012345678L ),
			new byte[] { -107, -122, 43, 57, 107, -52, 1, -122, 102, 0, 26, 64, -121, 1, -74, -101, 75, -90, 48, -13, 78, -127 } );

		// Date[]:
		assertValueToBytes( new Date[] { new Date( 123456789012345678L ), new Date( 234567890123456789L ) },
			new byte[] { -111, -107, -122, 43, 57, 107, -52, 1, 2, -107, -122, 43, 57, 107, -52, 1, -122, 102, 0, 26, 64, -121, 1, -74, -101, 75, -90, 48, -13, 78, -127, -107, -122, 43, 57, 107, -52, 1, -122, 102, 0, 26, 64, -121, 3, 65, 90, 64, -42, -123, -127, 21, -127, -127 } );
	}
...
//			System.out.println( buf.avail() );
	...
	final ValueFactoryFake vf = new ValueFactoryFake();
