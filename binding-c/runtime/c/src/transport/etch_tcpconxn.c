...
int   etch_tcpclient_on_data (etch_connection*, const int, int, etch_flexbuffer*);
int   etch_tcpconx_transport_data     (etch_tcp_connection*, etch_who*, etch_flexbuffer*);
int   etch_tcpconx_set_socket_options (etch_tcp_connection*);
int   etch_tcpconx_transport_control  (etch_tcp_connection*, etch_event*, objmask*);
int   etch_tcpconx_transport_notify   (etch_tcp_connection*, etch_event*);
objmask* etch_tcpconx_transport_query (etch_tcp_connection*, etch_query*); 
void  etch_tcpclient_set_session      (etch_tcp_connection*, void*);
void* etch_tcpclient_get_session      (etch_tcp_connection*); 
char* ETCHTCPX = "TCPX";
...
etch_tcp_connection* new_tcp_connection (etch_url* url, void* resources, etch_rawsocket* socket)
{
    ...
    {   if (-1 == (result = init_etch_tcpconx_interfaces(newcon))) break;
        ...
        {   etch_unicode_to_ansi (&newcon->cx.hostname, url->host);  
...
int init_etch_tcpconx_interfaces (etch_tcp_connection* tcpx)
{
    ...
    tcpx->itd = new_transportdata_interface (tcpx, 
        etch_tcpconx_transport_data, itransport);  /* itd now owns itransport */
...
int etch_tcpconx_open (etch_tcp_connection *conx, const int is_reconnect)
{
    ...
    {   if (cx->is_started) break;
        ...
        {   /* possibly todo: configure number of retry attempts */
            /* open socket */
            if (0 == (arc = apr_socket_connect (cx->socket, cx->sockdata)))
            {   cx->is_started = TRUE;
                /* possibly todo: set socket options here per APR */
                break;
            }
    ...
    if (cx->waiter) /* is another thread blocking on this condition? */
    {   /* when another thread is blocking on this condition variable,  
         * we set the condition to UP, which is presumably what the 
         * waiting thread will unblock on.
         */
        ...
        if (0 == result) /* fyi: conx->session->thisx is packetizer */
            conx->session->session_notify (conx->session->thisx, 
                new_etch_event(0, ETCHEVT_SESSION_UP));  
...
int etch_tcpconx_closex (etch_tcp_connection* conx, const int is_linger, const int is_dtor)
{
    ...
    if (0 != cx->mutex->tryacquire(cx->mutex))  
    ...
    {   /* if another thread is blocking on this condition variable,   
        ...
        ETCH_ASSERT(cx->waiter->obj_type == ETCHTYPEB_WAIT); 
 ...
 */
int destroy_etch_tcp_connection (etch_tcp_connection* tcpx)
{
    if (NULL == tcpx) return -1;
    if (tcpx->refcount > 0 && --tcpx->refcount > 0) return -1;  
    ...
    {
        ETCHOBJ_DESTROY(tcpx->rcvlxr);        /* free listener if any */

        etch_destroy_connection (&tcpx->cx);  /* free mem owned by tcpx */ 

        if (tcpx->is_session_owned)           /* free session interface */
            ETCHOBJ_DESTROY(tcpx->session);

        ETCHOBJ_DESTROY(tcpx->itd);           /* free transport interface */
    }

    tcpx->cx.on_event(tcpx, ETCH_CONXEVT_DESTROYED, 0, 0);
    return destroy_objectex((objmask*)tcpx);
}
...
{
   return apr_socket_create (outsock, APR_INET, SOCK_STREAM, APR_PROTO_TCP, mempool);
} 
 ...
 * etch_tcpclient_send()
 */
int etch_tcpclient_send (etch_tcp_connection *conx, char* buf, const size_t totallen, int* rc)
{
 ...
 */
int etch_tcpclient_sendex (etch_tcp_connection *conx, char* buf, 
    const size_t totallen, const int timeout_ms, int* rc)
{
    ...
    while(cx->is_started && !is_eod && remaining > 0)
    {
        ...
        is_eod = (APR_EOF == (arc = apr_socket_send(socket, buf + totalsent, &datalen)));
 ...
 */
int etch_tcpclient_receive (etch_tcp_connection *tcpx, char* buf, const size_t buflen, int* rc)
{
 ...
 */
int etch_tcpclient_receivex (etch_tcp_connection *tcpx, 
    char* buf, const size_t buflen, const int timeout_ms, int* rc)
{
    ...
    is_eod  = (APR_EOF == (arc = apr_socket_recv (cx->socket, buf, &datalen)));
    ...
    {    
        ...
        { case APR_OTHER_END_CLOSED: 
          ...
          case APR_THIS_END_CLOSED:  
               ...
               break;
          default: eventid = ETCH_CONXEVT_RECEIVERR; result = -1;
        } 
 ...
 */
int destroy_etch_tcp_client(etch_tcp_client* thisx)
{
    ETCHOBJ_DESTROY(thisx->threadpool);
...
{
    etch_tcp_client* newclient = (etch_tcp_client*) new_object 
        (sizeof(etch_tcp_client), ETCHTYPEB_TCPCLIENT, CLASSID_TCP_CLIENT);
    
    newclient->destroy  = destroy_etch_tcp_client;
    ...
    if (NULL == newclient->threadpool->run 
       (newclient->threadpool, etch_tcpclient_listenerproc, tcpx))
 ...
 */
int etch_tcpclient_on_data (etch_connection* cx, const int unused, int length, etch_flexbuffer* data)
{
    int result = 0;
    ...
    if (-1 == (result = session->session_data (session->thisx, NULL, data)))
        etchlog(ETCHCONX, ETCHLOG_ERROR, "current %d bytes on connxn %d rejected\n", 
                length, cx->conxid);
    return result;
}
 ...
 */
int etch_tcpclient_listenerproc (etch_threadparams* params)
{ 
	int result = 0, arc = 0;
    ...
    fbuf->destroy(fbuf);  
    etchlog(ETCHTCPX, ETCHLOG_DEBUG, "leaving listener thread ...\n");   
    return result;
}
 ...
 */
int etch_tcpconx_transport_data (etch_tcp_connection* thisx, etch_who* whoto, etch_flexbuffer* fbuf)
{
 ...
 */
void* etch_tcpclient_get_session (etch_tcp_connection* thisx) 
{
    ETCH_ASSERT(is_etch_tcpconnection(thisx));
 ...
 */
void etch_tcpclient_set_session (etch_tcp_connection* thisx, void* newsession)
{
    ETCH_ASSERT(is_etch_tcpconnection(thisx));
    ETCH_ASSERT(is_etch_sessiondata(newsession));
    if (thisx->is_session_owned)
        ETCHOBJ_DESTROY(thisx->session);
 ...
 */
int etch_tcpconx_transport_control (etch_tcp_connection* thisx, etch_event* control, objmask* value)
{
 ...
 */
int etch_tcpconx_transport_notify (etch_tcp_connection* thisx, etch_event* evt)
{
    ETCH_ASSERT(is_etch_tcpconnection(thisx));
 ...
 */
objmask* etch_tcpconx_transport_query (etch_tcp_connection* thisx, etch_query* query) 
{
    int result = 0;
    etchwait* waiter = NULL;
 ...
 */
int etch_tcpconx_set_socket_options(etch_tcp_connection *c)
{
    int arc = 0, ecount = 0;
